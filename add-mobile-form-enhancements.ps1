# Add mobile form field enhancements to all HTML files
# This script adds the mobile form enhancements CSS file to all HTML files
...
foreach ($file in $htmlFiles) {
    ...
    $cssLink = "<link rel=`"stylesheet`" href=`"$($pathPrefix)projects/stanley-black-decker/mobile-form-enhancements.css`">"
    
    # Check for existing CSS link
    $cssExists = $content -match "mobile-form-enhancements\.css"
    
    # Only proceed if the CSS link needs to be added
    if (!$cssExists) {
        # Try to add after the mobile tap targets CSS if it exists
        if ($content -match "mobile-tap-targets\.css") {
        ...
        Write-Host " - SKIPPED (Already has mobile form enhancements CSS)" -ForegroundColor Cyan
    }
}
...
if ($modifiedFiles -gt 0) {
    Write-Host "`nAdded mobile form field enhancements to $modifiedFiles files." -ForegroundColor Green
    Write-Host "All form fields will now have improved usability on mobile devices!" -ForegroundColor Green
} else {
