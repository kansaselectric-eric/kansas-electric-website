# Add responsive typography styles to all HTML files
# This script adds the responsive typography CSS file to all HTML files
...
foreach ($file in $htmlFiles) {
    ...
    $cssLink = "<link rel=`"stylesheet`" href=`"$($pathPrefix)projects/stanley-black-decker/responsive-typography.css`">"
    
    # Check for existing CSS link
    $cssExists = $content -match "responsive-typography\.css"
    
    # Only proceed if the CSS link needs to be added
    if (!$cssExists) {
        # Try to add after the responsive media CSS if it exists
        if ($content -match "responsive-media\.css") {
        ...
        Write-Host " - SKIPPED (Already has responsive typography CSS)" -ForegroundColor Cyan
    }
}
...
if ($modifiedFiles -gt 0) {
    Write-Host "`nAdded responsive typography styles to $modifiedFiles files." -ForegroundColor Green
    Write-Host "All text will now be at least 16px on mobile devices for better readability!" -ForegroundColor Green
} else {
